...
export class TimelineManager {
    ...
    constructor(private context: vscode.ExtensionContext) {
        this.storageFile = path.join(context.globalStorageUri.fsPath, 'timeline.json');
    ...
    private saveTimeline() {
        try {
            fs.writeFileSync(this.storageFile, JSON.stringify(this.timeline, null, 2));
        } catch (error) {
    ...
    public recordChange(document: vscode.TextDocument) {
        ...
        const changeCount = this.calculateChangeCount(previousContent, currentContent);
        ...
        if (changeCount > 0 || !this.previousContent.has(filePath)) {
            const entry: TimelineEntry = {
                ...
                diff: previousContent ? this.createDiff(previousContent, currentContent) : undefined
            };
    ...
    public getTimelineData() {
        ...
        const groups: { [key: number]: { timestamp: number, changes: number, entries: TimelineEntry[] } } = {};
    ...
    public async restoreSnapshot(entry: TimelineEntry) {
        try {
            ...
            vscode.window.showInformationMessage(
                `Restored ${entry.fileName} to ${new Date(entry.timestamp).toLocaleString()}`
            );
        } catch (error) {
